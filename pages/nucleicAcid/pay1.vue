	...
	<div class="container">
		<view class="top" v-for="(item,index) in appoints" :key="index">
			<view class="left">
				...
				<text style="font-size: 13px;color: darkgrey;">费用总额:<text style="color: red;">¥{{item.payFee}}</text></text>
				<text style="font-size: 13px;color: darkgrey;">就诊时间:{{item.payDate}}</text>
			</view>
			<view class="right">
				<button size="mini" style="background-color: dodgerblue;border-radius:30px;" type="primary" @click="toPay">去支付</button>
			</view>
		</view>
