...
<script>
	...
	export default {
		...
		onShow: function() {
			...
			this.familySign = getApp().globalData.familySign
		},
		methods: {
			// 放大二维码
			...
			modify: function(fId) {
				this.fId = fId
				uni.redirectTo({
					url: './modifyFamily?fId=' + this.fId
				})
			},
			toClickFamily: function(e) {
				console.log("=====listfamily=====",this.familySign)
				if(this.familySign=='01'){  		//从确认挂号跳转到就诊人页面
					getApp().globalData.familySign = ""
					getApp().globalData.fId = e.currentTarget.dataset.secretfid
					uni.redirectTo({
						url: '/pagesA/confirmGuahao/confirmGuahao'
					})
				}else if(this.familySign=='02'){   //从首页处跳转到就诊人页面
					getApp().globalData.familySign = ""
					getApp().globalData.fId = e.currentTarget.dataset.secretfid
					uni.switchTab({
						url: '/pages/index/index'
					})
				}else if(this.familySign=='03'){
					getApp().globalData.familySign = ""
					getApp().globalData.fId = e.currentTarget.dataset.secretfid
					uni.redirectTo({
