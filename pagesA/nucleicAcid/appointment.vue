<template>
	<div class="info">
		...
		<view class="item">
			...
			<view class="right">
				<text>{{family[index].secretName}}</text>
			</view>
		</view>
...
<script>
	import {
		AppointServlet,AskerServlet
	} from '@/pages/common/api.js'
	import api from '@/pages/common/request.js'
	import {
		randomNum,getClientNowSystemTime,decrypt
	...
	export default {
		...
		created() {
			...
			for(let i =0;i<arr.length;i++){
				arr[i].secretAid = decrypt(arr[i].secretAid)
				arr[i].secretName = decrypt(arr[i].secretName)
				this.appoints.push(arr[i].secretName+" "+arr[i].secretAid)
			}
			this.family =arr
		},
		methods:{
			...
			// 			}
			// 			for(let i =0;i<res.length;i++){
			// 				res[i].secretAid = decrypt(res[i].secretAid)
			// 				res[i].secretName = decrypt(res[i].secretName)
			// 				this.appoints.push(res[i].secretName+" "+res[i].secretAid)
			// 			}
			// 			this.family =res
			// 		}
			...
			submit(){
				...
				var data = {
					...
					appStatus: '1',
					fId:this.family[this.index].secretAid,
					openId: openid,
					paySatus:'0',
					FLAG: "C"
				}
				...
				api.post(AppointServlet, data).then(res => {
					...
					if(code =="200"){
						...
						setTimeout(function () {
							uni.hideLoading();
							uni.navigateTo({
								url:'/pagesA/nucleicAcid/success?appNo='+ data.appNo + '&appType=' + data.appType
							})
						}, 2000);
					}else {
						uni.showToast({
